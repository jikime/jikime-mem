  ...
  code-review:
    ...
    steps:
      ...
      - name: Run automated code review
        run: |
          python -m jikime_adk.code_review \
            --path . \
...
code_review:
  ...
  image: python:3.10
  script:
    - pip install pylint flake8 bandit mypy jikime-adk
    - python -m jikime_adk.code_review --path . --output review-report.json --format json
  artifacts:
    paths:
      - review-report.json
    reports:
...
quality_gate:
  ...
  script:
    - pip install jikime-adk
    - python -m jikime_adk.quality_gate --report review-report.json --fail-on-violation
  dependencies:
    - code_review
  allow_failure: false
  only:
