...
## Example 1: DDD 사이클 실행 (ANALYZE-PRESERVE-IMPROVE)
...
**Input**:
```python
from jikime_workflow_testing import AutomatedCodeReviewer
  ...
  "auto_fixable": [
    {
      "file": "src/auth/service.py",
      "fix": "Add type hints",
      "apply_command": "jikime-workflow fix --file src/auth/service.py --type hints"
    }
  ]
}
```
...
```python
from jikime_workflow_testing import QualityGateManager
...
```bash
# 전체 워크플로우 실행
jikime-workflow execute --project /project/src --mode full

# 개별 컴포넌트 실행
jikime-workflow debug --file app.py --error "AttributeError"
jikime-workflow refactor --directory src/ --max-risk medium
jikime-workflow profile --target function_name --types cpu,memory
jikime-workflow test --spec user_auth.spec --mode ddd
jikime-workflow review --project /project/src --trust-score-min 0.8

# CI 모드
jikime-workflow ci --commit abc123 --quality-gates strict
```
