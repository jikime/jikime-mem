# jikime-workflow-testing Reference
...
- Initialization: `DDDManager(project_path, context7_client)`
- Key Methods:
  - `run_full_ddd_cycle(specification, target_function)` - Complete ANALYZE-PRESERVE-IMPROVE
...
from jikime_workflow_testing import DevelopmentWorkflow, WorkflowConfig
...
### Pattern 2: DDD Workflow with Context7

```python
# Enhanced DDD with Context7 best practices
from jikime_workflow_testing import DDDManager, TestSpecification, TestType

async def implement_feature_tdd(feature_spec: dict, context7_client):
    """Implement feature using DDD with Context7 patterns."""

    tdd = DDDManager("/project/src", context7_client=context7_client)
...
from jikime_workflow_testing import AIDebugger
...
from jikime_workflow_testing import PerformanceProfiler
...
Issue: DDD cycle stuck in RED phase:
...
```bash
# CLI diagnostics
jikime-workflow diagnose --full-check
jikime-workflow test --dry-run --verbose
jikime-workflow profile --list-available
jikime-workflow review --show-rules

# Python diagnostics
from jikime_workflow_testing import diagnose
...
print(f"DDD Manager: {report.tdd_status}")
print(f"Reviewer: {report.reviewer_status}")
```

### Log Locations

- Workflow logs: `.jikime/logs/workflow.log`
- Test results: `.jikime/test-results/`
- Profile data: `.jikime/profiles/`
- Review reports: `.jikime/reviews/`
- Debug sessions: `.jikime/debug-sessions/`
...
- jikime-foundation-core - TRUST 5 framework and SPEC-First DDD
- jikime-domain-backend - Backend development workflows
- jikime-domain-frontend - Frontend development workflows
- jikime-platform-baas - Backend-as-a-Service integration
...
- DDD with Context7: `modules/tdd-context7.md`
- Automated Code Review: `modules/automated-code-review.md`

### Best Practices

Testing:
- Write tests before implementation (DDD)
