---
name: jikime-foundation-context
aliases: [jikime-foundation-context]
...
replaces: jikime-core-context-budget, jikime-core-session-state
...
Mandatory /clear Points: After /jikime:1-plan completion to save 45-50K tokens. When context exceeds 150K tokens to prevent overflow. When conversation exceeds 50 messages to remove stale history. Before major phase transitions for clean slate. During model switches for Haiku to Sonnet handoffs.
...
- jikime-cc-memory - Memory management and context persistence
- jikime-cc-configuration - Session configuration and preferences
- jikime-core-workflow - Workflow state persistence and recovery
- jikime-cc-agents - Agent state management across sessions
- jikime-foundation-trust - Quality gate integration
...
SPEC-First Workflow: Execute /jikime:1-plan, then mandatory /clear to save 45-50K tokens, then /jikime:2-run SPEC-XXX, then multi-agent handoffs with Pattern 4, then /jikime:3-sync SPEC-XXX, then session state persistence with Pattern 3.
