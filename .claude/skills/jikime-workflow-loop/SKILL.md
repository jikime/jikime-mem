...
- /jikime:alfred: One-click Plan-Run-Sync automation
- /jikime:loop: Start feedback loop
- /jikime:fix: One-time auto-fix
...
The Ralph Engine follows a layered architecture. User commands such as /jikime:loop, /jikime:fix, and /jikime:alfred enter the Command Layer. The Command Layer invokes the Hook System, which contains the PostToolUse Hook for LSP diagnostics and the Stop Hook for loop control. The Hook System connects to Backend Services including the LSP Client (JikiMELSPClient), AST-grep Scanner, and Test Runner. Backend Services feed into Completion Check which evaluates whether errors are zero, tests pass, and coverage is met. Based on the Completion Check result, the system either continues the loop or completes.
...
The Stop hook is triggered after each Claude response. The hook reads the loop state file located at .jikime/cache/.jikime_loop_state.json. This state contains active status (true or false), current iteration number, max_iterations limit, last_error_count from previous iteration, and completion_reason when finished.
...
Initialize the JikiMELSPClient from jikime_adk.lsp.client with the project_root parameter set to the project directory path.
...
For GitHub Actions integration, create a workflow step that runs Claude with the /jikime:loop command and max-iterations flag. Set the JIKIME_LOOP_ACTIVE environment variable to true to enable loop mode.
...
Check that ralph.enabled is set to true in configuration. Verify JIKIME_DISABLE_LOOP_CONTROLLER environment variable is not set. Ensure the state file location is writable.

### LSP Diagnostics Missing

Check LSP server configuration in .lsp.json file. Verify the language server is installed for your language. Check that JIKIME_DISABLE_LSP_DIAGNOSTIC environment variable is not set.

### Loop Stuck

Review the max_iterations setting to ensure it allows sufficient iterations. Review completion conditions to verify they are achievable. Send any message to interrupt the loop, or delete the state file (.jikime/cache/.jikime_loop_state.json) to reset.

## Works Well With

Skills:

- jikime-foundation-quality: TRUST 5 validation
- jikime-tool-ast-grep: Security scanning patterns
- jikime-workflow-testing: DDD integration
- jikime-lang-python: Python-specific patterns
...
JIKIME_DISABLE_LSP_DIAGNOSTIC disables the LSP hook when set.

JIKIME_DISABLE_LOOP_CONTROLLER disables the loop hook when set.

JIKIME_LOOP_ACTIVE indicates whether the loop is currently active.

JIKIME_LOOP_ITERATION contains the current iteration number.

CLAUDE_PROJECT_DIR contains the project root path.

### File Locations

Configuration is stored at .jikime/config/sections/ralph.yaml.

Loop state is stored at .jikime/cache/.jikime_loop_state.json.

The LSP hook is located at .claude/hooks/jikime/post_tool__lsp_diagnostic.py.

The loop hook is located at .claude/hooks/jikime/stop__loop_controller.py.
...
Skill Name: jikime-workflow-loop (formerly jikime-ralph)
