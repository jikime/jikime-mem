...
/jikime:loop --max-iterations 5

# Start with specific completion criteria
/jikime:loop --zero-warnings true
```
...
/jikime:fix src/auth.py

# Fix with specific severity threshold
/jikime:fix --severity error
```
...
ralph:
  ...
  ast_grep:
    ...
    quality_scan: true
    custom_rules:
      - .jikime/ast-grep/security/**/*.yml
      - .jikime/ast-grep/quality/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .jikime/ast-grep/react/**/*.yml
      - .jikime/ast-grep/typescript/**/*.yml
```
...
def load_loop_state() -> dict:
    state_file = Path(".jikime/cache/.jikime_loop_state.json")
...
```yaml
# .jikime/ast-grep/security/sql-injection.yml
...
```yaml
# .jikime/ast-grep/security/xss-prevention.yml
id: xss-react
language: typescript
rule:
...
```yaml
# .jikime/ast-grep/security/hardcoded-secrets.yml
      ...
      - name: Run Ralph Loop
        run: |
          claude -p "/jikime:loop --max-iterations 5" \
            ...
            --output-format json
        env:
          JIKIME_LOOP_ACTIVE: "true"
          CLAUDE_API_KEY: ${{ secrets.CLAUDE_API_KEY }}

      - name: Check Results
        run: |
          python .jikime/scripts/check_ralph_results.py
```
    ...
    steps:
      ...
      - name: Run ${{ matrix.check }} Check
        run: |
          case "${{ matrix.check }}" in
            lsp)
              claude -p "/jikime:fix --severity error" \
                --allowedTools "Read,Write,Edit"
              ;;
            ast-grep)
              jikime ast-grep scan --security
              ;;
            tests)
              pytest tests/ -v
              ;;
      ...
      - name: Upload Results
        ...
        with:
          ...
          path: .jikime/reports/${{ matrix.check }}
```
...
# Run LSP diagnostics
jikime lsp diagnose --changed-files

if [ $? -ne 0 ]; then
    echo "❌ LSP errors found. Run '/jikime:fix' to resolve."
    exit 1
fi

# Run AST-grep security scan
jikime ast-grep scan --security --changed-files
...
# Install JikiME-ADK
RUN pip install jikime-adk

# Copy project
COPY . .

# Initialize JikiME-ADK
RUN jikime init

# Run Ralph quality check
RUN claude -p "/jikime:loop --max-iterations 3" \
    --allowedTools "Read,Write,Edit" \
...
```bash
# View LSP logs
cat .jikime/logs/lsp_diagnostic.log

# Test LSP connection
jikime lsp test-connection python

# View diagnostics directly
jikime lsp diagnose src/auth.py
```
...
```bash
# View current state
cat .jikime/cache/.jikime_loop_state.json

# Reset loop state
rm .jikime/cache/.jikime_loop_state.json

# View loop logs
cat .jikime/logs/loop_controller.log
```
...
Check:
  ...
  - JIKIME_DISABLE_LOOP_CONTROLLER not set
  - State file writable
