...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/jikime-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
| ------------------------------ | ------- | -------------------------------------- | -------------------------------- |
| `JIKIME_DISABLE_LSP_DIAGNOSTIC`  | boolean | Disable LSP diagnostic hook            | `JIKIME_DISABLE_LSP_DIAGNOSTIC=1`  |
| `JIKIME_DISABLE_LOOP_CONTROLLER` | boolean | Disable loop controller hook           | `JIKIME_DISABLE_LOOP_CONTROLLER=1` |
| `JIKIME_LOOP_ACTIVE`             | boolean | Loop active flag (set by commands)     | `JIKIME_LOOP_ACTIVE=1`             |
| `JIKIME_LOOP_ITERATION`          | integer | Current iteration number               | `JIKIME_LOOP_ITERATION=3`          |
...
export JIKIME_DISABLE_LSP_DIAGNOSTIC=1
...
export JIKIME_LOOP_ACTIVE=1
export JIKIME_LOOP_ITERATION=0
claude -p "/jikime:loop --max-iterations 5"
```
...
```python
JikiMELSPClient(project_root: str | Path)
```
...
```python
from jikime_adk.lsp.client import JikiMELSPClient

client = JikiMELSPClient(project_root="/path/to/project")
```
...
```python
from jikime_adk.lsp.models import Position
...
```python
from jikime_adk.lsp.models import DiagnosticSeverity
...
```python
from jikime_adk.lsp.models import Range, Position
...
```python
from jikime_adk.lsp.models import Diagnostic, DiagnosticSeverity, Range, Position
...
```python
from jikime_adk.lsp.models import Location, Range, Position
...
```python
from jikime_adk.lsp.models import TextEdit, Range, Position
...
```python
from jikime_adk.lsp.models import WorkspaceEdit, TextEdit, Range, Position
...
```python
from jikime_adk.lsp.models import HoverInfo, Range, Position
...
### Loop State File (.jikime_loop_state.json)

Location: `.jikime/cache/.jikime_loop_state.json`
...
Location: `.claude/skills/jikime-tool-ast-grep/rules/sgconfig.yml`
...
- `/jikime:loop` command does nothing
...
1. Check if Ralph is enabled:
   ...
   ```yaml
   # .jikime/config/sections/ralph.yaml
   ralph:
     enabled: true
   ```
...
3. Check environment variable:
   ...
   ```bash
   unset JIKIME_DISABLE_LOOP_CONTROLLER
   ```

4. Verify state file is writable:
   ```bash
   mkdir -p .jikime/cache
   chmod 755 .jikime/cache
   ```
...
4. Check environment variable:
   ...
   ```bash
   unset JIKIME_DISABLE_LSP_DIAGNOSTIC
   ```
...
3. Delete state file:
   ```bash
   rm .jikime/cache/.jikime_loop_state.json
   ```
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/jikime-tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@jikime-adk.dev"
          git add .
...
```python
# .claude/hooks/jikime/custom_completion_check.py
