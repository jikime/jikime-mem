...
!git log --oneline -10
!find .jikime/specs -name "spec.md" -type f 2>/dev/null
...
# JikiME-ADK Step 3: Document Synchronization (+Optional PR Ready)
...
- `/jikime:3-sync` → Auto-sync (PR Ready only)
- `/jikime:3-sync --auto-merge` → PR auto-merge + branch cleanup
- `/jikime:3-sync force` → Force full synchronization
- `/jikime:3-sync status` → Check synchronization status
- `/jikime:3-sync project` → Integrated project synchronization
- `/jikime:3-sync auto src/auth/` → Specific path synchronization
- `/jikime:3-sync --worktree` → Sync in worktree mode (with worktree exit options)
- `/jikime:3-sync --branch` → Sync in branch mode (with branch management options)
...
/jikime:3-sync performs documentation synchronization through complete agent delegation:

Execution Flow:

- User Command: /jikime:3-sync [mode] [path]
- /jikime:3-sync Command delegates to Task with subagent_type set to manager-docs or manager-quality or manager-git
...
2. [HARD] JikiME-ADK Structure Must Exist:

   Requirement: `.jikime/` directory must exist in project root
...
4. [SOFT] Handle Worktree Detection:
   ...
   - Alternative: Check worktree registry at `~/worktrees/{PROJECT_NAME}/.jikime-worktree-registry.json`
...
2. [HARD] Create Backup Directory:
   ...
   Action: Execute `mkdir -p .jikime-backups/sync-$TIMESTAMP/`
...
3. [HARD] Backup All Critical Project Files:
   ...
   - .jikime/specs/ directory - SPEC definitions
...
4. [HARD] Verify Backup Integrity:
   ...
   Actions:
   - Execute: `ls -la .jikime-backups/sync-$TIMESTAMP/`
...
5. Generate synchronization report:
   - File location: .jikime/reports/sync-report-$TIMESTAMP.md
...
1. Batch update all completed SPECs:
   ...
   ```bash
   python3 .claude/hooks/jikime/spec_status_hooks.py batch_update
   ```
...
3. Handle individual SPEC validation (if needed):
   ...
   ```bash
   python3 .claude/hooks/jikime/spec_status_hooks.py validate_completion <SPEC_ID>
   python3 .claude/hooks/jikime/spec_status_hooks.py status_update <SPEC_ID> --status completed --reason "Documentation synchronized successfully"
   ```
...
- Project integrity: [PASS / WARNING]
 ...
 Reports Generated:
- Master sync report: .jikime/reports/sync-report-$TIMESTAMP.md
- Domain reports: [list if any]

 Backup Location:
- Safety backup: .jikime-backups/sync-$TIMESTAMP/
...
2. Execute user choice:
   - IF "Return to Main Directory" → Execute: `cd ~/JikiME/JikiME-ADK`
   ...
   - IF "Remove This Worktree" → Execute: `jikime-worktree remove {CURRENT_SPEC_ID}` then return to main
...
- `options`: 3-4 choices depending on context:
  - " Create Next SPEC" → /jikime:1-plan
...
- Reports: `.jikime/reports/sync-report-{timestamp}.md`
- Backup: `.jikime-backups/sync-{timestamp}/` (safety backup)
...
Coverage Target: Read from .jikime/config/sections/quality.yaml (constitution.test_coverage_target)
        ...
        "options": [
            {
                ...
                "description": "Execute /jikime:1-plan to plan new feature"
            },
            {
