...
!find .jikime/specs -name "\*.md" -type f 2>/dev/null
...
# JikiME-ADK Step 1: Establish a plan (Plan) - Always make a plan first and then proceed
...
- Use JikiME manager-spec agent for SPEC generation

Phase 1B: Specialized Analysis

- Use JikiME domain agents (expert-backend, expert-database, etc.) for specialized decisions
...
/jikime:1-plan performs SPEC planning through complete agent delegation:

Execution Flow:

- User Command: /jikime:1-plan "description"
- /jikime:1-plan Command delegates to Task with subagent_type set to Explore or manager-spec
...
Command Scope: Only executes Steps 1-2. Steps 3-4 are executed by `/jikime:2-run` and `/jikime:3-sync`.
...
2. SPEC Candidate Generation: Create 1-3 SPEC candidates
   - Analyze existing SPECs in `.jikime/specs/` for duplicates
...
- Note: Detailed versions finalized in `/jikime:2-run` stage
...
- question: "Planning is complete. Would you like to proceed with SPEC creation based on this plan?"
  ...
  - label: "Proceed with SPEC Creation"
    description: "Create SPEC files in .jikime/specs/SPEC-{ID}/ based on approved plan"
...
- Directory: .jikime/specs/SPEC-[ID]/
...
1. Create directory: `.jikime/specs/SPEC-{ID}/`
2. Save plan to `.jikime/specs/SPEC-{ID}/plan.md` with status: draft
3. Create commit: `draft(spec): WIP SPEC-{ID} - {title}`
4. Print to user: "Draft saved. Resume with: `/jikime:1-plan resume SPEC-{ID}`"
...
Format requirement: `.jikime/specs/SPEC-{ID}/`
...
- Use Glob(".jikime/specs/\*_/SPEC-_.md") to check existing SPEC list directly
...
Verification scope: Search entire .jikime/specs/ directory before creation
...
Correct approach: Create `.jikime/specs/SPEC-AUTH-001/` as directory
Incorrect approach: Create `.jikime/specs/SPEC-AUTH-001.md` as single file
...
- Create all 3 files at once:
  - .jikime/specs/SPEC-{SPEC_ID}/spec.md
  - .jikime/specs/SPEC-{SPEC_ID}/plan.md
  - .jikime/specs/SPEC-{SPEC_ID}/acceptance.md
...
- Read the configuration file from .jikime/config/config.yaml
...
Note: PR creation is handled separately in /jikime:2-run or /jikime:3-sync (Team mode only)

Expected Outcome:

- Feature branch created: feature/SPEC-[SPEC_ID]-description
- Current branch switched to feature branch
- Ready for implementation in /jikime:2-run
...
- Ready for /jikime:2-run implementation
...
1. Stage SPEC files: `git add .jikime/specs/SPEC-{SPEC_ID}/`
...
- Set worktree root to the user home directory under worktrees/JikiME-ADK
...
- Provide next steps guidance:
  - Option 1: Switch to worktree using jikime-worktree switch command
  - Option 2: Use shell eval with jikime-worktree go command
  - Option 3: Run /jikime:2-run with the SPEC ID
...
- If SPEC commit fails:
  ...
  - Suggest manual commit: `git add .jikime/specs/SPEC-{ID}/ && git commit -m "feat(spec): Add SPEC-{ID}"`
...
- Status: DRAFT (awaiting review)
 ...
 Next Steps:
1.  Review SPEC in `.jikime/specs/SPEC-{SPEC_ID}/`
2.  Execute `/jikime:2-run SPEC-{SPEC_ID}` to begin implementation
...
6.  Finalize PR in `/jikime:3-sync` when complete
:ELSE:
4.  Create PR in `/jikime:3-sync` when implementation complete
}
```
...
3.  Make commits directly to current branch
4.  Follow DDD: ANALYZE → PRESERVE → IMPROVE cycles
5.  Create PR in `/jikime:3-sync` when implementation complete
```
...
- Path: .jikime/worktrees/JikiME-ADK/SPEC-{SPEC_ID}/
- Branch: feature/SPEC-{SPEC_ID}
- Base Commit: Contains committed SPEC files
- Status: Ready for parallel development

 Next Steps:
1.  Switch to worktree: `jikime-worktree switch SPEC-{SPEC_ID}`
2.  Or use shell eval: `eval $(jikime-worktree go SPEC-{SPEC_ID})`
3.  Review SPEC documents in worktree: `.jikime/specs/SPEC-{SPEC_ID}/`
4.  Execute `/jikime:2-run SPEC-{SPEC_ID}` to begin DDD implementation
...
- [ ] Directory naming correct: `.jikime/specs/SPEC-{ID}/` format followed
...
- Entry Point: /jikime:1-plan "feature description"
...
- Entry Point: /jikime:1-plan "vague request"
...
- Entry Point: /jikime:1-plan resume SPEC-XXX
- Key Phases: Phase 1B then Phase 2 then Phase 3
...
- Mode: NEW
- Entry Point: /jikime:1-plan "feature" --worktree
...
- Mode: User Choice
- Entry Point: /jikime:1-plan "feature" (prompt_always: true)
...
- Mode: Automated
- Entry Point: /jikime:1-plan "feature" (prompt_always: false, auto_enabled: true)
...
- Command: /jikime:1-plan "User authentication system" --worktree
- Output: SPEC created: SPEC-AUTH-001, Worktree created: ~/worktrees/JikiME-ADK/SPEC-AUTH-001
- Next steps:
  1. Switch to worktree: jikime-worktree switch SPEC-AUTH-001
  2. Or use shell eval: eval $(jikime-worktree go SPEC-AUTH-001)
...
- Command: /jikime:1-plan "Payment integration"
...
- WorktreeManager Import: `from jikime_adk.cli.worktree.manager import WorktreeManager`
- Worktree Registry: Automatic registration in `~/worktrees/JikiME-ADK/.jikime-worktree-registry.json`
...
- Location: `.jikime/specs/SPEC-{ID}/` (directory format, NOT single .md file)
...
1. --worktree Flag Support: Added argument parsing for --worktree flag in /jikime:1-plan
2. WorktreeManager Integration: Automatic worktree creation using existing src/jikime_adk/cli/worktree/manager.py
...
Command execution: /jikime:1-plan "User authentication" --worktree
...
- Import: from jikime_adk.cli.worktree.manager import WorktreeManager
- Worktree Registry: Automatic registration in ~/worktrees/JikiME-ADK/.jikime-worktree-registry.json
...
- Start Implementation - Execute /jikime:2-run to begin DDD development
