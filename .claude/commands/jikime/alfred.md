...
@.jikime/config/sections/git-strategy.yaml
@.jikime/config/sections/quality.yaml
@.jikime/config/sections/llm.yaml

---

# /jikime:alfred - Agentic AI Autonomous Automation
...
AI: All issues resolved
  ↓
AI: <jikime>DONE</jikime>
```

## Command Purpose

Autonomously execute the full JikiME workflow:
...
5. **Completion Marker Detection** (`<jikime>DONE</jikime>`)
...
/jikime:alfred "Add JWT authentication"

# Enable auto loop (max 50 iterations)
/jikime:alfred "JWT auth" --loop --max 50

# Sequential exploration + auto loop
/jikime:alfred "JWT auth" --loop --sequential

# Resume previous work
/jikime:alfred resume SPEC-AUTH-001
```
...
All implementation complete, tests passing, docs updated. <jikime>DONE</jikime>
```
...
```
START: /jikime:alfred "task description"
...
/jikime:alfred "SQL query optimization"
...
/jikime:alfred "task" --loop --sequential

# Specify max iterations
/jikime:alfred "task" --loop --max 50

# Branch + PR
/jikime:alfred "task" --branch --pr

# Resume
/jikime:alfred resume SPEC-XXX
```
