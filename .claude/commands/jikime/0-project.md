...
# JikiME-ADK Step 0: Generate Project Documentation
...
Note: Project configuration (language, service, pricing, git mode) is handled by `jikime init` CLI command.
...
WHY: Users installing JikiME-ADK into an empty directory need to describe their project. Users with existing code need automatic analysis.
...
When user selects "Existing Project" and `.jikime/project/` is empty:
