...
# JikiME-ADK Step 2: Execute Implementation (Run) - DDD Implementation
...
The `/jikime:2-run` command orchestrates the complete implementation workflow by delegating to specialized agents rather than performing tasks directly.

**SPEC ID Parameter**: Supply via `$ARGUMENTS` (e.g., `/jikime:2-run SPEC-001`)

---

## Execution Philosophy: "Plan to Run to Sync"

The `/jikime:2-run` command executes SPEC implementation through sequential phase-based agent delegation.
...
- Checkpoint: User directed to /jikime:3-sync
...
- `/jikime:2-run SPEC-XXX` (retry same command)
...
- Guide user toward `/jikime:3-sync` as recommended next step
...
- stats:
  ...
  - commits: Count
- next_step_options:
  - Option 1: Sync Documentation - Execute /jikime:3-sync
  - Option 2: Implement Another Feature - Return to /jikime:1-plan
...
- Provide four options:
  - "Sync Documentation" - Execute /jikime:3-sync to synchronize documentation
  - "Implement Another Feature" - Execute /jikime:1-plan to define another SPEC
...
- **Implement new SPEC feature**: `/jikime:2-run SPEC-XXX`
...
- **Resume after interruption**: `/jikime:2-run SPEC-XXX` (retry same command)
  - Resumes from last successful phase checkpoint
...
- Recommend `/jikime:3-sync` as primary next step
  - WHY: Maintains workflow momentum with documentation synchronization
...
1. **Sync Documentation** (recommended)
   - Action: Execute `/jikime:3-sync` to synchronize documentation and create PR
...
2. **Implement Another Feature**
   - Action: Return to `/jikime:1-plan` to define additional SPEC
...
- commits: 3
- options:
  - Option 1 (primary): Sync Documentation - Execute /jikime:3-sync to organize documentation and create PR
  - Option 2: Implement Another Feature - Define and implement additional SPEC, action /jikime:1-plan
...
- Use conversation language from `.jikime/config/config.yaml`
