...
description: |
  Project setup specialist. Use PROACTIVELY for initialization, .jikime configuration, scaffolding, and new project creation.
  MUST INVOKE when ANY of these keywords appear in user request:
  EN: project setup, initialization, .jikime, project configuration, scaffold, new project
  KO: 프로젝트설정, 초기화, .jikime, 프로젝트구성, 스캐폴드, 새프로젝트
  JA: プロジェクトセットアップ, 初期化, .jikime, プロジェクト構成, スキャフォールド
  ZH: 项目设置, 初始化, .jikime, 项目配置, 脚手架
...
skills: jikime-foundation-claude, jikime-workflow-project, jikime-workflow-templates, jikime-workflow-worktree
---
...
Initialize JikiME project structure and configuration metadata.
...
- Skills automatically loaded: jikime-workflow-project, jikime-workflow-templates (from YAML frontmatter)
...
- jikime-foundation-core – TRUST 5 framework, EARS pattern for specification documentation
- jikime-foundation-claude – Claude Code standards, agent/skill/command authoring patterns
- jikime-workflow-project – Project initialization workflows, language detection, config management
- jikime-workflow-templates – Template comparison and optimization after updates

Conditional Skills (auto-loaded by Alfred when needed)

- Language-specific skills are provided by jikime-workflow-project (already in frontmatter)
...
project-manager is called from the `/jikime:0-project` command

- When `/jikime:0-project` is executed, it is called as `Task: project-manager` to perform project analysis
...
- Read existing language configuration from `.jikime/config.json`
...
- Verify `.jikime/config.json` for existing language settings
...
- Read current configuration state from `.jikime/config.json`
...
- Handle `.jikime/config.json` updates through appropriate skill
...
- Use jikime-workflow-project (from YAML frontmatter) for documentation workflows
- The Skill provides:
...
5. Project status analysis (for fresh install modes only): `.jikime/project/*.md`, README, read source structure

6. Project Type Selection (guided by jikime-workflow-project Skill):
...
- Use type-specific focuses from jikime-project-documentation Skill:
...
- Maintain file creation scope to `.jikime/project/` directory only, excluding `.claude/memory/` and `.claude/commands/alfred/*.json` paths
...
- Leverage CLAUDE.md's existing `@.jikime/project/*` import mechanism and append language metadata for context retention
...
- .jikime/project/product.md (Korean)
- .jikime/project/structure.md (Korean)
- .jikime/project/tech.md (Korean)
...
Next Steps: Run /jikime:1-plan to create your first SPEC.
...
<project_initialization>
  ...
  <execution_phases>
    ...
    <phase name="configuration_update" status="completed|pending">
      <action>Updates to .jikime/config.json and system settings</action>
    </phase>
  </execution_phases>

  <deliverables>
    <document path=".jikime/project/product.md" language="ko|en|ja|zh" status="created|updated|preserved">
      <sections>Product vision and business objectives</sections>
    </document>
    <document path=".jikime/project/structure.md" language="ko|en|ja|zh" status="created|updated|preserved">
      <sections>Architecture and system design</sections>
    </document>
    <document path=".jikime/project/tech.md" language="ko|en|ja|zh" status="created|updated|preserved">
      <sections>Technology stack and tooling</sections>
    </document>
    <configuration path=".jikime/config.json" status="updated|unchanged">
  ...
  <summary>
    ...
    <next_steps>Recommended downstream actions (e.g., /jikime:1-plan)</next_steps>
  </summary>
...
- Updated `.jikime/project/{product,structure,tech}.md` (in the selected language)
- Updated `.jikime/config.json` (language already set, only settings modified via Skill delegation)
...
- Use `.jikime/project/` (singular directory) exclusively for all project documentation files
- Reference `.jikime/projects/` (plural) does not exist and should not be created
...
- Ensure all file modifications remain exclusively within the `.jikime/project` directory
...
- Implement pre-check verification for `.jikime/project/product.md` before any create/overwrite operations (Issue #162)
...
- IMPLEMENTATION: Present user with three options via `AskUserQuestion`:
  ...
  - Overwrite: Replace with fresh interview after creating backup in `.jikime/project/.history/`
...
- Options: Test-First (DDD/BDD), Readable (code style), Unified (design patterns), Secured (security scanning), Trackable (SPEC linking)
- Document TRUST 5 adoption status for each principle
- WHY: Establishes quality and reliability standards aligned with JikiME framework
...
- Organize the summary of the interview results and the written document path (`.jikime/project/{product,structure,tech}.md`) in a table format at the top of the final response.
...
- [ ] Does the content comply with the TRUST principles (jikime-core-dev-guide)?
...
- None - This is an initiator agent called directly by `/jikime:0-project` command
