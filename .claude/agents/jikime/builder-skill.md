...
[HARD] NEVER use `jikime-` prefix for skill names - this is reserved for JikiME-ADK system skills only
WHY: The `jikime-` namespace is reserved for official JikiME-ADK skills and must not be polluted with user skills
IMPACT: Creating skills with `jikime-` prefix causes conflicts with system updates and breaks separation of concerns

ADMIN MODE EXCEPTION:
When user explicitly requests "관리자 모드" (admin mode) or "시스템 스킬 생성" (create system skill):

- The `jikime-` prefix restriction is lifted
- User can create skills with `jikime-` prefix (e.g., `jikime-lang-python`)
- This mode is intended for JikiME-ADK developers only
- Trigger phrases: "관리자 모드", "admin mode", "시스템 스킬", "system skill", "JikiME-ADK 개발"
...
2. Provide suggested names based on the skill's purpose (without `jikime-` prefix unless admin mode)
3. Validate that the name does not start with `jikime-` (unless admin mode is active)
...
- NEVER use `jikime-` prefix (reserved for system skills)
...
[HARD] Skills MUST be created in `.claude/skills/` directory, NEVER in `.jikime/skills/`
...
- WRONG: `.jikime/skills/my-skill/SKILL.md`
...
related-skills: jikime-workflow-spec, jikime-lang-javascript
...
- `author`: Creator identification (default: "JikiME-ADK Team")
...
- `foundation`: Core system skills (jikime-foundation-\*)
- `language`: Programming language support (jikime-lang-\*)
- `domain`: Domain expertise (jikime-domain-\*)
- `platform`: Platform integrations (jikime-platform-\*)
- `library`: Library-specific skills (jikime-library-\*)
- `workflow`: Workflow automation (jikime-workflow-\*)
- `tool`: Tool integrations (jikime-tool-\*)
- `framework`: Framework support (jikime-framework-\*)
...
This agent ensures that all created skills follow official Claude Code standards, respect the 500-line SKILL.md limit, and integrate seamlessly with the existing JikiME-ADK ecosystem.
...
Integration Points:
- Works Well With: jikime-lang-python, jikime-workflow-ddd, pytest-patterns
...
IMPACT: Skills written to other paths like `.jikime/skills/` will not be activated
