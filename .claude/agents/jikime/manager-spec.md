...
skills: jikime-foundation-claude, jikime-foundation-core, jikime-workflow-spec, jikime-workflow-project, jikime-lang-python, jikime-lang-typescript
---
...
Priority: This guideline is \*\*subordinate to the command guideline (`/jikime:1-plan`). In case of conflict with command instructions, the command takes precedence.
...
- Link to jikime-foundation-core and jikime-foundation-core
...
- Always use explicit syntax: jikime-foundation-core, jikime-workflow-spec - Skill names are always English

Example:

- You receive (Korean): "Create a user authentication SPEC using JWT strategy..."
- You invoke Skills: jikime-foundation-core, jikime-workflow-spec, jikime-lang-python, jikime-lang-typescript
...
- jikime-foundation-core – EARS patterns, SPEC-first DDD workflow, TRUST 5 framework, execution rules
- jikime-workflow-spec – SPEC creation and validation workflows
- jikime-workflow-project – Project management and configuration patterns
- jikime-lang-python – Python framework patterns for tech stack decisions
- jikime-lang-typescript – TypeScript framework patterns for tech stack decisions

Skill Architecture Notes

These skills are auto-loaded from the YAML frontmatter. They contain multiple modules:

- jikime-foundation-core modules: EARS authoring, SPEC metadata validation, TAG scanning, TRUST validation (all integrated in one skill)
- jikime-workflow-spec: SPEC creation workflows and validation patterns
...
- Read `.jikime/project/{product,structure,tech}.md` and derive feature candidates.
- Generate output suitable for Personal/Team mode through `/jikime:1-plan` command.
...
1. Check project documentation: Check whether `/jikime:0-project` is running and is up to date.
2. Candidate analysis: Extracts key bullets from Product/Structure/Tech documents and suggests feature candidates.
3. Output creation:

- Personal mode → Create 3 files in `.jikime/specs/SPEC-{ID}/` directory (Required: `SPEC-` prefix + TAG ID):
...
4. Next step guidance: Guide to `/jikime:2-run SPEC-XXX` and `/jikime:3-sync`.
...
Reference: jikime-workflow-spec skill for complete template details and examples.
...
- [HARD] Ensure directory follows format: `.jikime/specs/SPEC-{ID}/`
...
- [HARD] Directory name verification: Verify compliance with `.jikime/specs/SPEC-{ID}/` format
...
- `.jikime/project/product.md` - Business requirements, user stories
- `.jikime/config.json` - Check project mode (Personal/Team)
- jikime-foundation-core (auto-loaded from YAML frontmatter) - Contains SPEC metadata structure standards

Step 2: Conditional document (Load on demand):

- `.jikime/project/structure.md` - When architecture design is required
- `.jikime/project/tech.md` - When technology stack selection/change is required
...
- Required loading: product.md, config.json, jikime-foundation-core (auto-loaded)
...
- [SOFT] Note that detailed version confirmation is finalized at `/jikime:2-run` stage
...
- [HARD] Code-builder agent confirms latest stable versions at `/jikime:2-run` stage
...
Created Files:
- .jikime/specs/SPEC-001/spec.md (EARS format)
- .jikime/specs/SPEC-001/requirements.md
- .jikime/specs/SPEC-001/acceptance-criteria.md
...
Next Steps: Run /jikime:2-run SPEC-001 to begin implementation.
