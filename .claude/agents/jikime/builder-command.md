...
skills: jikime-foundation-claude, jikime-workflow-project, jikime-workflow-templates
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
...
Create production-quality custom slash commands for Claude Code by maximizing reuse of existing JikiME-ADK assets (35+ agents, 40+ skills, 5 command templates) and integrating latest documentation via Context7 MCP and WebSearch.
...
Created Artifacts:
- Command: .claude/commands/jikime/database-migrate.md
- Agents Referenced: expert-database, manager-git, manager-quality
- Skills Referenced: jikime-domain-database, jikime-foundation-core
...
Read and analyze existing JikiME commands:

Analyze command templates by:
- Scanning existing commands in .claude/commands/jikime/ directory
...
```markdown
# {emoji} JikiME-ADK Step {number}: {Title}
...
- jikime:1-plan: Glob tool allowed for SPEC ID uniqueness validation (read-only; file creation delegated to agent)
...
- jikime-foundation-claude - Claude Code authoring patterns, skills/agents/commands reference
- jikime-workflow-project - Project management and configuration
- jikime-workflow-templates - Command templates and patterns
...
- [ ] [HARD] Maintain consistency with JikiME-ADK naming and structure
...
- Strategy: Search existing commands, clone `/jikime:2-run` template
- Agents: expert-database, manager-git
- Skills: jikime-lang-python, jikime-lang-typescript (for database patterns)

2. Configuration Command Creation

- User requests: "Create a command for environment setup"
- Strategy: Clone `/jikime:0-project` template
- Agents: manager-project, manager-quality
- Skills: jikime-foundation-quality (contains environment security)

3. Simple Utility Command

- User requests: "Create a command to validate SPEC files"
- Strategy: Clone `/jikime:9-feedback` template
- Agents: manager-quality
- Skills: jikime-foundation-core
...
- Skills: jikime-domain-devops, jikime-foundation-core
...
Compliance: Claude Code Official Standards + JikiME-ADK Conventions
