...
skills: jikime-foundation-claude, jikime-foundation-core, jikime-foundation-philosopher, jikime-workflow-spec, jikime-workflow-project, jikime-workflow-worktree, jikime-lang-python, jikime-lang-typescript
---
...
3. **Technical Terms in English** [HARD]:
   - Skill names (example: jikime-core-language-detection, jikime-domain-backend)
...
- You invoke: jikime-core-language-detection, jikime-domain-backend
...
- jikime-language-support – Automatically branches execution strategies for each language when planning.
- jikime-foundation-philosopher – Strategic thinking framework for complex decisions (always loaded for this agent).

Conditional Skill Logic

- jikime-foundation-claude: Load when this is a multi-language project or language-specific conventions must be specified.
- jikime-essentials-perf: Called when performance requirements are included in SPEC to set budget and monitoring items.
- jikime-core-tag-scanning: Use only when an existing TAG chain needs to be recycled or augmented.
- Domain skills (`jikime-domain-backend`/`frontend`/`web-api`/`mobile-app`, etc.): Select only one whose SPEC domain tag matches the language detection result.
- jikime-core-trust-validation: Called when TRUST compliance measures need to be defined in the planning stage.
...
- **Read SPEC Directory Structure** [HARD]:
  - Each SPEC is a **folder** (e.g., `.jikime/specs/SPEC-001/`)
...
1. Locate the SPEC folder in `.jikime/specs/SPEC-{ID}/` directory
...
- For SPEC-001: Read `.jikime/specs/SPEC-001/spec.md`, `.jikime/specs/SPEC-001/plan.md`, `.jikime/specs/SPEC-001/acceptance.md`
...
- workflow-spec: Create SPEC file (`.jikime/specs/`)
...
This agent participates in the /jikime:2-run Phase chain. Context must be properly received and passed to maintain workflow continuity.

**Input Context** (from /jikime:2-run command):
...
- **SPEC Directory Structure**:
  - Location: `.jikime/specs/SPEC-{ID}/`
  - Files: `spec.md`, `plan.md`, `acceptance.md`
  - Example: `.jikime/specs/SPEC-001/spec.md`
- Development guide: jikime-core-dev-guide
- TRUST principles: TRUST section in jikime-core-dev-guide
- TAG Guide: TAG Chain section in jikime-core-dev-guide
