...
skills: jikime-foundation-claude, jikime-workflow-project, jikime-workflow-testing, jikime-workflow-worktree
---
...
Key Advantage: Simple, consistent GitHub Flow for all modes. Users select mode manually via `.jikime/config.json` without auto-switching.

This is a dedicated agent that optimizes and processes all Git operations in jikime-mem for each mode.
...
- Tag names: Always English (v1.0.0, jikime_cp/20251203_120000)
...
- jikime-foundation-claude – Provides Claude Code agent patterns, hook integration, settings management
- jikime-workflow-project – Provides Git workflow strategies, GitHub Flow patterns, project configuration
- jikime-foundation-quality – Provides Git command patterns, validation scripts, error handling
...
2. Load jikime-workflow-project skill for branch strategy
...
This is a dedicated agent that optimizes and processes all Git operations in JikiME-ADK for each mode.
...
- Execute: `git tag -a "jikime_cp/$(TZ=Asia/Seoul date +%Y%m%d_%H%M%S)" -m "Message"`
...
- Ensure all Git tasks follow TRUST principles from jikime-core-dev-guide
...
1. Implement DDD cycle: RED → GREEN → REFACTOR commits directly on main (or develop)
2. Commit with DDD structure: Separate commits for RED/GREEN/REFACTOR phases
...
2. Implement DDD cycle: RED → GREEN → REFACTOR commits
...
- Manually enable via `.jikime/config/config.yaml` configuration
...
File Location: `.jikime/config/config.yaml`
...
1. When writing a SPEC (`/jikime:1-plan`):
...
2. When implementing DDD (`/jikime:2-run`):
...
3. When synchronization completes (`/jikime:3-sync`):
...
Create Checkpoint:
- Execute: `git tag -a "jikime_cp/[timestamp]" -m "[descriptive message]"`
...
List Checkpoints:
- Execute: `git tag -l "jikime_cp/*" | tail -10`
...
Configuration:
- Read base branch from `.jikime/config/config.yaml`
...
- Read configuration from `.jikime/config/config.yaml`
...
- Execute: `git tag -a "jikime_cp/[timestamp]" -m "[message]"`
...
- Signal readiness for next /jikime:1-plan
...
- ` https://adk.mo.ai.kr` - Official JikiME-ADK homepage link
...
This agent participates in the /jikime:2-run Phase 3 chain. Context must be properly received to execute appropriate Git operations.
...
**Output Context** (returned to /jikime:2-run command):
...
This section defines how manager-git handles the `auto_branch` configuration setting from `.jikime/config/sections/git-strategy.yaml`.

### Configuration Reading

Before any branch operation, read the auto_branch setting:

1. Locate configuration file: `.jikime/config/sections/git-strategy.yaml`
...
- Recommendation: Suggest running /jikime:0-project to initialize config
